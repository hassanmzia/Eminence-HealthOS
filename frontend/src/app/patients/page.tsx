  ...
  return (
    <div className="space-y-6 bg-mesh min-h-full">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          { label: "Total", value: patients.length, color: "text-gray-900", bg: "bg-gray-50" },
      ...
      {loading ? (
        ...
        <div className="card flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="mt-4 text-sm font-medium text-gray-900">No patients found</p>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <p className="text-sm font-semibold text-gray-900 group-hover:text-healthos-700">{name}</p>
                        {patient.mrn && (
                          <p className="text-xs text-gray-400">MRN: {patient.mrn}</p>
                        )}
                      </div>
              ...
              {filtered.map((patient, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-3.5 text-sm text-gray-500">{patient.mrn || "—"}</td>
                    <td className="px-6 py-3.5">
                      <div className="flex gap-1">
                        {conditions.slice(0, 2).map((c) => (
                          <span key={c} className="badge-neutral">{c}</span>
                        ))}
                        {conditions.length > 2 && <span className="text-xs text-gray-400">+{conditions.length - 2}</span>}
                      </div>
                    </td>
                    <td className="px-6 py-3.5 text-sm tabular-nums text-gray-500">{patient.medications?.length || 0}</td>
                    <td className="px-6 py-3.5">
                      <span className={cfg.class}>{patient.risk_level}</span>
                    </td>
                    <td className="px-6 py-3.5">
