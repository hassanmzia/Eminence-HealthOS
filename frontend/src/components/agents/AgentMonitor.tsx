...
const STATUS_COLORS: Record<string, { dot: string; badge: string }> = {
  idle: { dot: "bg-gray-400", badge: "bg-gray-100 text-gray-700" },
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        {activityLoading ? (
          <div className="card py-8 text-center text-sm text-gray-400">Loading activity...</div>
        ) : !activity || activity.executions.length === 0 ? (
          <div className="card py-8 text-center text-sm text-gray-400">No recent agent executions</div>
        ) : (
          <div className="card overflow-hidden p-0">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-2 text-center text-xs font-medium uppercase text-gray-500">Status</th>
                  <th className="px-4 py-2 text-right text-xs font-medium uppercase text-gray-500">Duration</th>
                ...
                {activity.executions.map((ex) => (
                  <tr key={ex.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2.5">
                      <span className="text-sm font-medium text-gray-900">{ex.agent_name}</span>
                    </td>
                    <td className="px-4 py-2.5 text-xs text-gray-500">{ex.action}</td>
                    <td className="px-4 py-2.5 text-center">
                      <StatusBadge status={ex.status} />
                    </td>
                    <td className="whitespace-nowrap px-4 py-2.5 text-right text-xs text-gray-600">
      ...
      {activity && activity.pipeline_runs.length > 0 && (
        <div>
          <h2 className="mb-2 text-lg font-semibold text-gray-900">Pipeline Activity</h2>
                ...
                <div className="flex items-start justify-between">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-gray-500">{run.trigger_event}</span>
                    </div>
