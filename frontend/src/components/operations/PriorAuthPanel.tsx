  ...
  return (
    <div className="card">
      <div className="mb-3 flex items-center justify-between">
        <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Prior Authorizations</h2>
      ...
      <div className="mb-4 grid grid-cols-2 sm:grid-cols-4 gap-2">
        {Object.entries(counts).map(([status, count]) => (
          <div key={status} className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2 text-center">
            <div className="text-lg font-bold text-gray-900 dark:text-gray-100">{count}</div>
            <div className="text-xs capitalize text-gray-500 dark:text-gray-400">{status}</div>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-2">
        {auths.map((auth) => (
          <div key={auth.auth_reference} className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3">
            <div className="flex items-center justify-between">
              <div>
                <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{auth.procedure}</span>
                <span className="ml-2 text-xs text-gray-500 dark:text-gray-400">{auth.auth_reference}</span>
              </div>
