#!/usr/bin/env python3
"""
Eminence HealthOS - Qdrant Vector Store Initializer
...
def get_embedding(text: str, model: str = EMBEDDING_MODEL) -> Optional[list]:
    """Generate an embedding using OpenAI. Returns None if not configured."""
...
def seed_collections(
    ...
    logger.info("=" * 60)
    logger.info("Eminence HealthOS - Qdrant Initializer")
    logger.info("=" * 60)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Initialize HealthOS Qdrant vector store collections"
    )
    parser.add_argument(
        "--url",
        default=QDRANT_URL,
