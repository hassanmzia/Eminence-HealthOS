"""
Eminence HealthOS — Patient-related Cypher queries for the Neo4j medical knowledge graph.
Async adaptations of InHealth patient graph queries.
"""
...
async def create_or_update_patient_node(patient_id: str, properties: Dict[str, Any]) -> Dict[str, Any]:
    ...
    RETURN p
    """
    result = await run_write_query(cypher, {"patient_id": patient_id, "properties": properties})
    return result[0] if result else {}


async def link_patient_to_condition(
    patient_id: str, icd10_code: str, onset_date: str | None = None
) -> None:
...
async def link_patient_to_medication(
    patient_id: str, rxnorm_code: str, start_date: str | None = None
) -> None:
...
async def get_patient_comorbidity_network(patient_id: str) -> List[Dict[str, Any]]:
    """
    ...
    LIMIT 10
    """
    return await run_query(cypher, {"patient_id": patient_id})


async def get_patient_risk_factors_graph(patient_id: str) -> List[Dict[str, Any]]:
    """
...
async def find_similar_patients(
    patient_id: str, min_similarity: float = 0.7
) -> List[Dict[str, Any]]:
    """
    ...
    LIMIT 20
    """
    return await run_query(cypher, {"patient_id": patient_id, "min_similarity": min_similarity})
