...
class RandomForestDiseaseClassifier:
    ...
    via ``.dict()``) containing observations, conditions, etc.
    """
    ...
    def extract_features(self, patient_data: Dict[str, Any]) -> Dict[str, float]:
        ...
        Expected ``patient_data`` layout::

            {
                ...
                "gender": "male",
                "observations": [
                    {"code": "39156-5", "value": 28.5, "effective_datetime": ..., "status": "final"},
                    ...
                ],
                "conditions": [
                    {"code": "I21.9", "clinical_status": "active"},
                    ...
                ],
            }
        ...
        # Helper: latest observation by LOINC code
        def get_latest_obs(loinc_code: str, days_back: int = 180) -> Optional[float]:
            ...
            candidates = [
                ...
                and o.get("status") == "final"
                and o.get("effective_datetime") is not None
                and o["effective_datetime"] >= cutoff
            ]
            if not candidates:
                return None
            candidates.sort(key=lambda o: o["effective_datetime"], reverse=True)
            return float(candidates[0]["value"]) if candidates[0].get("value") is not None else None
        ...
        # Family history flags (defaults -- can be enriched)
        ...
        # Prior events from conditions
        conditions = set()
        for cond in patient_data.get("conditions", []):
            if cond.get("clinical_status") == "active" and cond.get("code"):
                conditions.add(cond["code"])
