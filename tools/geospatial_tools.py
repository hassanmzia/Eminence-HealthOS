"""
Eminence HealthOS -- Geospatial / Hospital Routing Tools

Re-exports the core ``find_nearest_hospital`` tool from ``base_tools`` and
adds route planning and multi-hospital search helpers.

No Django dependency -- configuration via Pydantic ``ToolSettings`` / env vars.
"""
...
from tools.base_tools import (
    _get_pg_conn,
    find_nearest_hospital,  # noqa: F401 -- re-export
    get_tool_settings,
)

logger = logging.getLogger("healthos.tools.geospatial")
