"""
Eminence HealthOS -- FHIR Database Query Tools

Re-exports the core ``query_fhir_database`` tool from ``base_tools`` and
provides additional FHIR-specific helpers for resource validation and
...
from tools.base_tools import query_fhir_database  # noqa: F401 -- re-export

logger = logging.getLogger("healthos.tools.fhir")

# Valid FHIR R4 resource types supported by HealthOS
...
def validate_fhir_resource_type(resource_type: str) -> dict:
    """
    Validate whether a FHIR resource type is supported by the HealthOS system.
...
FHIR_TOOLS = [validate_fhir_resource_type, get_patient_summary]
