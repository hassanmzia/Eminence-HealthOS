-- ============================================================
-- Eminence HealthOS - Performance Indexes
-- Reference schema for Alembic migrations
--
-- Advanced indexes beyond the basic ones created with each
-- table: GIN (jsonb_path_ops), trigram, partial, composite,
-- and pgvector ANN indexes.
-- ============================================================
...
-- ============================================================
-- GIN Indexes for JSONB Columns (jsonb_path_ops)
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_fhir_appt_upcoming
    ON fhir_appointment(subject_patient_id, start_time)
    WHERE status IN ('booked', 'arrived') AND start_time > NOW();
...
CREATE INDEX IF NOT EXISTS idx_care_gap_reporting
    ON care_gap(tenant_id, measure_id, status, measure_period_start, measure_period_end);
